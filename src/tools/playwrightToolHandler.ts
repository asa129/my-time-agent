...
export async function ensureBrowser(
  ...
  if (userCtx.get(INITIALIZING_KEY)) {
    console.log(
      "Browser initialization in progress for this context, waiting..."
    );
  ...
  let browserInstance = userCtx.get(BROWSER_INSTANCE_KEY) as
    | Browser
    | undefined;
  ...
  if (!browserInstance || !browserInstance.isConnected()) {
    try {
      ...
      console.log(
        `[${context.operationId}] Launching new browser instance for context...`
      );
      ...
      console.log(
        `[${context.operationId}] Browser launched successfully for context.`
      );
    } catch (error) {
      console.error(
        `[${context.operationId}] Failed to initialize browser for context:`,
        error
      );
    ...
    console.log(
      `[${context.operationId}] Creating new page in existing browser context...`
    );
    ...
    if (!currentContext) {
      ...
      console.error(
        `[${context.operationId}] Browser exists but has no context!`
      );
  ...
  if (!browserInstance || !pageInstance) {
    throw new Error(
      "Failed to ensure browser and page instances within context."
    );
  }
...
export async function resetBrowserState(
  ...
  console.log(
    `[${context.operationId}] Resetting browser state for context...`
  );
  ...
  console.log(
    `[${context.operationId}] Browser state reset complete for context.`
  );
}
