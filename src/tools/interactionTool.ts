...
export const typeTool = createTool({
  ...
  parameters: z.object({
    selector: z
      .string()
      .describe("CSS or XPath selector for the target element."),
...
export const selectOptionTool = createTool({
  ...
  execute: async (args, options?: ToolExecuteOptions) => {
    ...
    if (value !== undefined) selectCriteria = { value };
      ...
      throw new Error(
        "Either value, label, or index must be provided for selectOption."
      );
  ...
  execute: async (args, options?: ToolExecuteOptions) => {
    ...
    return safeBrowserOperation(
      ...
      async (page) => {
        ...
        return {
          result: `Hovered over element with selector: ${args.selector}`,
        };
      }
    );
  },
